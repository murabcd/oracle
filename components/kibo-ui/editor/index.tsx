...
import type { ComponentProps, FormEventHandler, HTMLAttributes, ReactNode } from "react";
import {
  Children,
  isValidElement,
  useCallback,
...
const defaultSlashSuggestions: SuggestionOptions<SuggestionItem>["items"] =
  () => [
    {
      title: "Text",
...
type EditorFloatingMenuProps = Omit<FloatingMenuProps, "editor">;
...
type EditorBubbleMenuProps = Omit<BubbleMenuProps, "editor">;
    ...
    >
      {(() => {
        ...
        return childArray.reduce((acc: ReactNode[], child, index) => {
          ...
          const previousKey = getEditorChildKey(
            childArray[index - 1],
            `previous-${index - 1}`
          );
          const currentKey = getEditorChildKey(child, `current-${index}`);

          acc.push(
            <Separator
              key={`separator-${String(previousKey)}-${String(currentKey)}`}
...
const getEditorChildKey = (child: ReactNode, fallback: string) =>
  isValidElement(child) && child.key != null ? String(child.key) : fallback;

type EditorClearFormattingProps = Pick<EditorButtonProps, "hideName">;

const EditorClearFormatting = ({
  hideName = true,
...
type EditorNodeTextProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeText = ({
  hideName = false,
...
type EditorNodeHeading1Props = Pick<EditorButtonProps, "hideName">;

const EditorNodeHeading1 = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeHeading2Props = Pick<EditorButtonProps, "hideName">;

const EditorNodeHeading2 = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeHeading3Props = Pick<EditorButtonProps, "hideName">;

const EditorNodeHeading3 = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeBulletListProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeBulletList = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeOrderedListProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeOrderedList = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeTaskListProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeTaskList = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeQuoteProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeQuote = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeCodeProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeCode = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorNodeTableProps = Pick<EditorButtonProps, "hideName">;

const EditorNodeTable = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorSelectorProps = Omit<HTMLAttributes<HTMLDivElement>, "children"> & {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  title: string;
  children?: ComponentProps<typeof PopoverContent>["children"];
};
...
type EditorFormatBoldProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatBold = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorFormatItalicProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatItalic = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorFormatStrikeProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatStrike = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorFormatCodeProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatCode = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorFormatSubscriptProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatSubscript = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorFormatSuperscriptProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatSuperscript = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
type EditorFormatUnderlineProps = Pick<EditorButtonProps, "hideName">;

const EditorFormatUnderline = ({
  hideName = false,
}: Pick<EditorButtonProps, "hideName">) => {
  const { editor } = useCurrentEditor();
...
const EditorTableMenu = ({ children }: EditorTableMenuProps) => {
  const { editor } = useCurrentEditor();
...
type EditorTableGlobalMenuProps = {
  children: ReactNode;
};

const EditorTableGlobalMenu = ({
  children,
...
type EditorTableRowMenuProps = {
  children: ComponentProps<typeof DropdownMenuContent>["children"];
};

const EditorTableRowMenu = ({ children }: EditorTableRowMenuProps) => {
  const { editor } = useCurrentEditor();
  const [top, setTop] = useState(0);
  const [left, setLeft] = useState(0);
...
const EditorTableHeaderColumnToggle = () => {
  const { editor } = useCurrentEditor();
...
const EditorTableHeaderRowToggle = () => {
  const { editor } = useCurrentEditor();
