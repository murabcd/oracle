import { generateLink } from "@/lib/link/server";
...
export const POST = async (request: Request) => {
  ...
  if (
    typeof body?.startingUrl !== "undefined" &&
    typeof body.startingUrl !== "string"
  ) {
    return Response.json(
      { error: "Starting URL must be a string" },
      { status: 400 }
    );
  }

  const response = await generateLink(body);
